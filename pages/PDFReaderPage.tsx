...
import { ChevronLeft, ChevronRight, ZoomIn, ZoomOut, ArrowLeft, Loader2, ExternalLink, AlertCircle, FileText, ScrollText, RectangleHorizontal } from 'lucide-react';
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                if (entry.isIntersecting) {
                    ...
                    // Optional: Set isVisible(false) to unload off-screen pages for memory (aggressive)
                    // For now, we keep them rendered or let simple logic handle it
                }
            },
            { rootMargin: '50% 0px', threshold: 0.1 } // Load when within 50% of viewport
        );
...
const PDFReaderPage = () => {
  ...
  // Handle Resize for Auto-Fit (Single Mode)
  useEffect(() => {
      const handleResize = () => {
          // Trigger re-calc logic if needed
      };
  ...
  useEffect(() => {
    if (viewMode !== 'single') return; // Only render in single mode
  ...
  const handleScrollPageVisible = useCallback((visiblePageNum: number) => {
      // Debounce updating page num to avoid jitter in input
      // Ideally we check if visiblePageNum is drastically different or if user stopped scrolling
      // For now, direct update is fine as long as we don't auto-scroll the view based on this state change immediately
  ...
  // --- Touch & Zoom Handlers (Single Mode) ---
  const handleTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      touchStartRef.current = { dist, scale: scale || 1 };
    }
  };
  ...
  const handlePageSubmit = (e: React.FormEvent) => {
      ...
      if (!isNaN(val) && val >= 1 && val <= numPages) {
          scrollToPage(val); // Use unified scroll/set handler
      } else {
          setPageInput(pageNum.toString());
      }
  ...
  const toggleViewMode = () => {
      ...
      setViewMode(newMode);
      // Wait for render then scroll to current page in new mode
      setTimeout(() => {
          if (newMode === 'scroll') {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-100/90 dark:bg-gray-900/90 backdrop-blur-sm flex flex-col h-full">
      ...
      {viewMode === 'single' ? (
          // --- Single Page View ---
          <div 
            className="flex-1 overflow-auto relative touch-pan-x touch-pan-y no-scrollbar pt-20 pb-24"
            ref={containerRef}
